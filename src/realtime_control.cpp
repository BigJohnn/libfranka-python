...
std::array<double, 7> RealtimeControl::applyRateLimiter(
    ...
    for (size_t i = 0; i < 7; i++) {
        ...
        double commanded_velocity = (commanded_positions[i] - last_positions[i]) / delta_t;
        
        // Calculate commanded jerk
        double commanded_jerk = (((commanded_velocity - last_velocities[i]) / delta_t) - last_accelerations[i]) / delta_t;
        
        // Limit jerk and integrate to get acceleration
        double commanded_acceleration = last_accelerations[i] + 
        ...
        double safe_max_acceleration = std::min(
            (max_jerk[i] / max_acceleration[i]) * (upper_velocity_limits[i] - last_velocities[i]),
            max_acceleration[i]);
        double safe_min_acceleration = std::max(
            (max_jerk[i] / max_acceleration[i]) * (lower_velocity_limits[i] - last_velocities[i]),
            -max_acceleration[i]);
        
        // Limit acceleration and integrate to get velocity
        double limited_velocity = last_velocities[i] +
            std::max(std::min(commanded_acceleration, safe_max_acceleration), safe_min_acceleration) * delta_t;
...
RealtimeControl::RealtimeControl(franka::Robot& robot) : robot_(robot) {
    ...
    control_thread_ = std::thread([this]() {
        try {
            ...
            motion_generator_ = std::make_unique<franka::MotionGenerator>(0.1, initial_state.q);
            ...
            while (!stop_thread_) {
                ...
                if (!motion_generator_ || target_position != last_target_) {
                    motion_generator_ = std::make_unique<franka::MotionGenerator>(0.5, target_position);
                ...
                if (motion_generator_) {
                    ...
                    if (rate_limiter_enabled_) {
                        for (size_t i = 0; i < 7; i++) {
                            output.q[i] = franka::lowpassFilter(duration.toSec(), output.q[i], state.q_d[i], 100);
                        }

                        std::array<double, 7> upper_velocity_limits = franka::computeUpperLimitsJointVelocity(state.q_d);
                        std::array<double, 7> lower_velocity_limits = franka::computeLowerLimitsJointVelocity(state.q_d);
            ...
            std::cerr << "[RealtimeControl] Control thread exception: " << e.what() << std::endl;
